...
- **YAML bare dates become ISO strings after VitePress serialization**: `date: 2026-01-01` (unquoted) is parsed by js-yaml as a `Date` object, then `JSON.stringify`-ed by VitePress into `"2026-01-01T00:00:00.000Z"`. On the client, `frontmatter.date` is that ISO string — not a `Date` instance. Splitting on `-` without slicing gives `["2026", "01", "01T00:00:00.000Z"]`; the third element becomes `NaN` → `Invalid Date`. Fix: `String(dateStr).slice(0, 10).split('-')` to safely extract `YYYY-MM-DD` from any format.
